...
iota.multisig.initiateTransfer(input, remainderAddress, multisigTransfer).then(function(initiatedBundle) {
  return iota.multisig.addSignature(initiatedBundle, address, iota.multisig.getKey('ABCDFG', 0, 3));
}).then(function(firstSignedBundle) {
  return iota.multisig.addSignature(firstSignedBundle, address, iota.multisig.getKey('FDSAG', 0, 3));
}).then(function(finalBundle) {
  console.log("IS VALID SIGNATURE: ", iota.utils.validateSignatures(finalBundle, address));
}).catch(function(e) {
  console.log(e);
});